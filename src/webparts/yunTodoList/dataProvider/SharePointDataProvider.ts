...
export default class SharePointDataProvider implements ITodoDataProvider {
  ...
  private _getItemsBatched(requester: SPHttpClientBatch): Promise<ITodoItem[]> {
    ...
    return requester
      .get(queryUrl, SPHttpClientBatch.configurations.v1)
      .then((response: SPHttpClientResponse) => {
        return response.json();
      })
      .then((json: { value: ITodoItem[] }) => {
  ...
  private _createItem(
    ...
    return batch.post(this._listItemsUrl, SPHttpClientBatch.configurations.v1, {
  ...
  private _deleteItem(
    ...
    return batch.fetch(itemDeletedUrl, SPHttpClientBatch.configurations.v1, {
      headers,
      method: "DELETE"
    });
  }
  ...
  private _updateItem(
    ...
    return batch.fetch(itemUpdatedUrl, SPHttpClientBatch.configurations.v1, {
      body: JSON.stringify(body),
      headers,
      method: "PATCH"
    });
  ...
  private _resolveBatch(
    ...
    return batch
      .execute()
      .then(() =>
        Promise.all(promises).then(values => values[values.length - 1])
      );
  }
}
